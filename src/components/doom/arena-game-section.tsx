...
export function ArenaGameSection() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (data.member) {
            ...
            loadLeaderboard();
            // Load referral code for share link
            loadReferralCode(data.member.handle);
  ...
  // Load referral code from engagement API (for share link)
  const loadReferralCode = useCallback(async (h: string) => {
    try {
      const res = await fetch("/api/engagement", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "get_referral_code", handle: h }),
      });
      if (res.ok) {
        const data = await res.json();
        if (data.referralLink) setMyReferralLink(data.referralLink);
      }
    } catch { /* silent */ }
  }, []);
                  ...
                  <motion.div key="game" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
                    ...
                    <div className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl p-4 sm:p-5">
                      ...
                      <div className="mt-3 flex gap-2">
                        <input readOnly value={myReferralLink || `https://doomhound.onrender.com/?ref=${member.handle}`}
                          className="flex-1 bg-[#0a0a0a] border border-[#2a2a2a] rounded-lg px-3 py-2 text-xs sm:text-sm text-gray-400 font-mono truncate" />
                        <button onClick={() => navigator.clipboard.writeText(myReferralLink || `https://doomhound.onrender.com/?ref=${member.handle}`)}
