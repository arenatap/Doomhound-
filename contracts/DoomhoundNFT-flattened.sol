...
contract DoomhoundNFT is ERC721, ERC721Enumerable, Ownable {
    ...
    // ===== ADMIN MINT (for giveaways/team — sequential token IDs) =====
    ...
    // ===== ADMIN MINT SPECIFIC TOKEN ID (for airdrop with exact token IDs) =====
    /**
     * @dev Mint a specific token ID to a recipient.
     * Used for airdropping the same token IDs from the old hacked contract.
     * Does NOT use the sequential counter — the token ID is explicitly set.
     *
     * @param to       Recipient address
     * @param tokenId  Specific token ID to mint (1-100)
     */
    ...
    /**
     * @dev Batch mint specific token IDs to a single recipient.
     * More gas-efficient than multiple adminMintToken calls.
     *
     * @param to        Recipient address
    ...
    function adminMintTokenBatch(address to, uint256[] calldata tokenIds) external onlyOwner {
        if (totalSupply() + tokenIds.length > MAX_SUPPLY) revert SoldOut();
